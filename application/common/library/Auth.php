...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'mobile', 'avatar', 'score', 'wall1', 'wall2'];
     ...
     * @return boolean
     */
    public function register($username, $password,$password2, $email = '', $mobile = '', $extend = [])
    {
        // 检测用户名或邮箱、手机号是否存在
...
//        }
        ...
        if(is_array($extend)){
            if(isset($extend['tjuser'])){
                $tjuser = User::getByUsername($extend['tjuser']);
                if(!$tjuser)  {
                    $this->setError('推荐人不存在');
                    return FALSE;
                }
                $extend['tjid'] = $tjuser['id'];
                $extend['ztlevel'] = $tjuser['ztlevel'] + 1;
                $extend['tpath'] = $tjuser['tpath'] . "," . $tjuser['id'];
            }
        }else{
            $extend['tpath'] = 0;
        }
    ...
    public function login($account, $password)
    {
        ...
        if (authcode($user->password,'DECODE') != $password)
        {
...
//        }
    ...
    public function changepwd($newpassword, $oldpassword = '', $ignoreoldpassword = false)
    {
        ...
        //判断旧密码是否正确
        if (authcode($this->_user->password,'DECODE') == $oldpassword || $ignoreoldpassword)
        {
     ...
     * @return boolean
     */
    public function changepwd2($newpassword, $oldpassword = '', $ignoreoldpassword = false)
    {
        if (!$this->_logined)
        {
            $this->setError('You are not logged in');
        ...
        //判断旧密码是否正确
        if (authcode($this->_user->password2,'DECODE') == $oldpassword || $ignoreoldpassword)
        {
            $newpassword = authcode($newpassword);
            $this->_user->save(['password2' => $newpassword]);
...
/**
    ...
    public function getUserinfo()
    {
        ...
        //dingzt新增
        $data = ['id' => Token::get($this->_token)['user_id']];
